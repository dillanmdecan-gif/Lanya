"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  EXPIRYRANGE BOT  v2  —  1HZ10V  —  Distribution Thinking                   ║
║                                                                              ║
║  PHILOSOPHY                                                                  ║
║  ──────────────────────────────────────────────────────────────────────────  ║
...
║  CONTRACT                                                                    ║
║  ──────────────────────────────────────────────────────────────────────────  ║
║  Symbol   : 1HZ10V (Volatility 10 Index)                                    ║
║  Type     : EXPIRYRANGE                                                     ║
...
║  ✅ Martingale: 1.75× multiplier after 2 losses, up to max steps, then reset ║
║     - Overrides Kelly while active                                           ║
║     - Initial (base) stake: $0.35 (flat minimum)                            ║
║     - Loss 1     → stays at base stake (martingale not yet active)          ║
║     - Loss 2     → stake × 1.75  (e.g. $0.35 → $0.61)                      ║
║     - Loss 3     → stake × 1.75² (e.g. $0.35 → $1.07)                      ║
║     - Loss 4     → stake × 1.75³ (e.g. $0.35 → $1.87)                      ║
║     - Any win at any step → resets to base stake                             ║
║  ✅ Tick rate tracker (1HZ10V is not perfectly 1Hz)                            ║
║                                                                              ║
║  DISTRIBUTION MODEL                                                          ║
║  ──────────────────────────────────────────────────────────────────────────  ║
║  1HZ10V price moves ≈ Gaussian random walk at short scales.                 ║
...
MARTINGALE_FACTOR     = _env("MARTINGALE_FACTOR",     1.75)    # ← 1.75× multiplier
...
# ─────────────────────────────────────────────────────────────────────────────
# TICK RATE TRACKER  (new for 1HZ10V — not guaranteed 1Hz)
# ─────────────────────────────────────────────────────────────────────────────

class TickRateTracker:
    """
    Measures the actual tick rate of 1HZ10V in real time.
    1HZ10V is approximately but not exactly 1 tick/second.
...
class OnlineEstimator:
    """
    Learns the price distribution of 1HZ10V in real time.
    ...
    Integrates TickRateTracker so N_ticks_expiry adjusts to actual 1HZ10V rate.
    """
...
class MartingaleManager:
    """
    1.75× martingale kicks in after 2 consecutive losses, up to max steps, then forced reset.

    Step 0  → base stake ($0.35)  [used for loss 1 as well — martingale not yet active]
    Step 1  → $0.35 × 1.75¹ ≈ $0.61  (activates on loss 2)
    Step 2  → $0.35 × 1.75² ≈ $1.07
    Step 3  → $0.35 × 1.75³ ≈ $1.87
    ...

    Any win resets to step 0 and clears the loss streak.
    After max steps loss, also resets (risk cap).
    ...
    def record_loss(self):
        ...
        # Martingale kicks in only after 2 losses; first loss stays at base stake
        if self.consecutive_losses < 2:
            mlog(f"LOSS #{self.consecutive_losses} → step {self.step} (martingale not yet active)")
            return
...
class DerivClient:
    ...
    async def place_trade(self, stake: float,
        ...
        """
        Place EXPIRYRANGE ±2.1 for DURATION_MIN minutes on 1HZ10V.
...
class ExpiryRangeBot:
    ...
    async def run(self):
        info("=" * 62)
        info("EXPIRYRANGE BOT  v2  —  1HZ10V  —  Distribution Thinking")
